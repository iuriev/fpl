// screens.jsx — FPL Squad Viewer (dark FPL palette + pitch layout)

// Data (extends starters/bench from screens.jsx if loaded, otherwise inline)
// stats object mirrors the FPL /event/{gw}/live/ stats payload exactly,
// per design-994f6b45.md §D3 (no translation layer).
function _s(pts, o) {
  return {
    total_points:     pts,
    minutes:          o.mp ?? 90,
    ...
    assists:          o.a  ?? 0,
    clean_sheets:     o.cs ?? 0,
    goals_conceded:   o.gc ?? 0,
    own_goals:        o.og ?? 0,
    penalties_saved:  o.ps ?? 0,
    penalties_missed: o.pm ?? 0,
    yellow_cards:     o.yc ?? 0,
    red_cards:        o.rc ?? 0,
    saves:            o.sv ?? 0,
    bonus:            o.b  ?? 0,
...
const STARTERS = [
  { pos: 'GK',  name: 'Raya',       club: 'ARS', pts: 6,  stats: _s(6,  { cs: 1, sv: 3, b: 1 }) },
  { pos: 'DEF', name: 'White',      club: 'ARS', pts: 4,  stats: _s(4,  { gc: 0, cs: 1 }) },
  { pos: 'DEF', name: 'Saliba',     club: 'ARS', pts: 8,  stats: _s(8,  { cs: 1, b: 1, a: 1 }) },
  { pos: 'DEF', name: 'Gabriel',    club: 'ARS', pts: 9,  stats: _s(9,  { g: 1, cs: 1, b: 1 }) },
  { pos: 'DEF', name: 'Mykolenko',  club: 'EVE', pts: 2,  status: 'i', stats: _s(2,  { mp: 0 }) },
  { pos: 'MID', name: 'Saka',       club: 'ARS', pts: 14, cap: 'C', stats: _s(14, { g: 1, a: 1, b: 3 }) },
  { pos: 'MID', name: 'Ødegaard',   club: 'ARS', pts: 7,  cap: 'V', stats: _s(7,  { a: 1, b: 0 }) },
  { pos: 'MID', name: 'Rodri',      club: 'MCI', pts: 10, stats: _s(10, { g: 1, b: 1 }) },
  { pos: 'MID', name: 'Martinelli', club: 'ARS', pts: 5,  status: 'd', stats: _s(5,  { mp: 67 }) },
  { pos: 'FWD', name: 'Watkins',    club: 'AVL', pts: 11, stats: _s(11, { g: 1, a: 1, b: 1 }) },
  { pos: 'FWD', name: 'Isak',       club: 'NEW', pts: 15, stats: _s(15, { g: 2, b: 3 }) },
];
const BENCH = [
  { pos: 'GK',  name: 'Flekken',     club: 'BRE', pts: 3, stats: _s(3, { mp: 90, gc: 2, sv: 4 }) },
  { pos: 'DEF', name: 'Trippier',    club: 'NEW', pts: 4, stats: _s(4, { mp: 90, gc: 1, yc: 1 }) },
  { pos: 'DEF', name: 'Pedro Porro', club: 'TOT', pts: 6, stats: _s(6, { mp: 90, a: 1 }) },
  { pos: 'MID', name: 'Mbeumo',      club: 'BRE', pts: 5, stats: _s(5, { mp: 90, a: 1 }) },
];
const TOTAL_PTS = STARTERS.reduce((s,p) => s + (p.cap === 'C' ? p.pts*2 : p.pts), 0);
const TRANSFER_HIT = 4;
const NET_POINTS = TOTAL_PTS - TRANSFER_HIT;
const SUMMARY = {
  total:     NET_POINTS,
  average:   51,
  highest:   142,
...
// ─── SQUAD HEADER / GW CONTROL ──────────────────────────────────────────────────
function SquadHeader({ teamName, loading }) {
  return (
    <div style={{
...
function GwControl({ gw, atLatest = true, loading = false }) {
  return (
    <div style={{
...
// ─── SQUAD SCREEN (pitch + list layouts) ────────────────────────────
// `view` is the persisted URL view mode (pitch | list) — see design-994f6b45.md.
function SquadScreen({ state = 'loaded', view = 'pitch', listScroll = 0 }) {
  if (state === 'loading') return <SquadScreenLoading view={view}/>;
  if (state === 'empty')   return <SquadScreenEmpty view={view}/>;
  return <SquadScreenLoaded view={view} listScroll={listScroll}/>;
}

function SquadScreenLoaded({ view = 'pitch', listScroll = 0 }) {
  return (
    <AppFrame>
      <StatusBar/>
      ...
      {view === 'list' ? (
        <div style={{ flex: 1, minHeight: 0 }}>
          <ListView starters={STARTERS} bench={BENCH} initialScroll={listScroll}/>
        </div>
      ) : (
        <div style={{ flex: 1, padding: '0 16px', minHeight: 0 }}>
          <Pitch starters={STARTERS} width={358} height={384}/>
...
function SquadScreenLoading({ view = 'pitch' }) {
  return (
    <AppFrame>
      <StatusBar/>
      <SquadHeader loading/>
      ...
      {view === 'list' ? (
        <div style={{ flex: 1, minHeight: 0 }}>
          <ListViewSkeleton rowCount={9}/>
        </div>
      ) : (
      <div style={{ flex: 1, padding: '0 16px', minHeight: 0 }}>
        <PitchSkeleton width={358} height={384}/>
        {/* bench skeleton */}
        <div style={{
...
function SquadScreenEmpty({ view = 'pitch' }) {
  return (
    <AppFrame>
      <StatusBar/>
      <SquadHeader teamName="Arteta's Army"/>
      <ViewToggle value={view}/>
      <GwControl gw={1} atLatest={false}/>
