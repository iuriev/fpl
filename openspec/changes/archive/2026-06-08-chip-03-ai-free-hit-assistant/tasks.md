...
- [x] P1. Add `selling_price` and `purchase_price` to `FPLPicksResponse.picks` in `proxy/src/fpl-client.ts`; update `getSquadPicks()` to pass them through to the squad summary so budget calculation can use sell prices.

- [x] P2. Create `proxy/src/free-hit-optimizer.ts` — greedy optimiser:
...
- [x] P3. Add route `GET /api/squads/:teamId/free-hit-suggest` in `proxy/src/squad-routes.ts` (or equivalent):
...
- [x] W1. Extend `TransferActionBar` props with `onAiFreeHit`, `isAiLoading`, `freehitAvailable`, `isPremium`; add a second CSS row with "AI Free Hit" and "AI Wildcard" buttons:
...
- [x] W2. Wire `handleAiFreeHit` in `TransferScreen`:
...
- [x] W3. Add copy strings to `web/src/lib/copy.ts`:
  `aiFreehitButton`, `aiWildcardButton`, `aiFreehitPlayed`, `aiFreehitError`, `aiFreehitNoGain`

## Tests

- [x] T1. Unit tests for `free-hit-optimizer.ts`:
...
- [x] T2. Update `TransferActionBar.test.tsx` (or snapshot) to cover the new AI row states
