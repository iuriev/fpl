...
- [x] `proxy/package.json`: add `resend` to `dependencies`.
- [x] `proxy/src/auth/auth.ts`:
...
- [x] `proxy/.env.example`: add `RESEND_API_KEY=re_...` and `FROM_EMAIL=noreply@...`.
- [x] `proxy/src/auth/auth.test.ts`: add test that `sendVerificationEmail` is called with
...
- [x] `docs/architecture.md`: add `RESEND_API_KEY` and `FROM_EMAIL` to the Fly secrets
...
- [x] `web/src/screens/VerifyEmailScreen/VerifyEmailScreen.tsx`: implement the screen
      (see design.md for state and render spec).
- [x] `web/src/screens/VerifyEmailScreen/VerifyEmailScreen.module.css`: styles consistent
      with `SignInScreen` / `SignUpScreen` layout.
- [x] `web/src/screens/VerifyEmailScreen/VerifyEmailScreen.test.tsx`: test render with
      user email displayed; test resend button triggers the correct API call; test error
      state on resend failure.
- [x] `web/src/screens/index.ts`: export `VerifyEmailScreen`.
- [x] `web/src/lib/copy.ts`: add `verifyEmailHeadline`, `verifyEmailBody`,
...
- [x] `web/src/App.tsx`: in `rootElement()`, after the `!user` check add:
      `if (user && !user.emailVerified) return <VerifyEmailScreen />;`.
      Add `/verify-email` named route rendering `<VerifyEmailScreen />`.
- [x] `web/src/screens/SignUpScreen/SignUpScreen.tsx`: after `authClient.signUp` succeeds,
      navigate to `/verify-email` (replace current `navigate('/', { replace: true })`).
- [x] `web/src/auth/auth-client.ts`: verify `AuthUser` type includes `emailVerified:
