...
- [ ] `web/src/lib/my-team/MyTeamContext.ts`: add `isDemoMode: boolean`,
...
- [ ] `web/src/lib/my-team/MyTeamProvider.tsx`: call `clearDemoMode()` is exported and
      usable by `SignInScreen`/`SignUpScreen` after successful auth.
- [ ] Update `web/src/lib/my-team/MyTeamContext.test.ts` (or provider test) to cover
...
- [ ] `web/src/App.tsx`: implement the routing state machine in `rootElement()`:
...
- [ ] `web/src/App.tsx`: replace `MyTeamProtectedRoute` with `AuthAndTeamProtectedRoute`
      that checks `user && user.fplTeamId`; redirects to `/sign-in` if no user, to `/entry`
      if user but no `fplTeamId`.
- [ ] `web/src/App.tsx`: add explicit `/entry` route rendering `EntryScreen`.
- [ ] `web/src/App.tsx`: remove `MyTeamProtectedRoute` function.
...
- [ ] `web/src/screens/EntryScreen/EntryScreen.test.tsx`: add tests for demo mode submit
...
- [ ] `web/src/screens/SignInScreen/SignInScreen.module.css`: style "Try Demo" as a
      tertiary/ghost button visually distinct from "Sign in with Google".
- [ ] `web/src/screens/SignUpScreen/SignUpScreen.tsx`: call `clearDemoMode()` on successful
      sign-up.
- [ ] `web/src/lib/copy.ts`: add `signInTryDemo` copy key.
- [ ] `web/src/screens/SignInScreen/SignInScreen.test.tsx`: test "Try Demo" navigation.
...
- [ ] `web/src/screens/SquadScreen/SquadScreen.tsx`: pass `navLinksMode` derived from
      `isDemoMode` and `isGuestMode`:
  - `isDemoMode` → `"demo"`
  - `isGuestMode` → `"hidden"`
  - otherwise → `"full"`
- [ ] `web/src/components/ui/TeamInfoPanel/TeamInfoPanel.test.tsx`: test all three modes
...
- [ ] Run `npm run test -w web` — all tests pass.
- [ ] Run `npm run typecheck -w web` — no type errors.
- [ ] Run `npm run lint -w web` — no lint errors.
